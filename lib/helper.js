...
const ElementWrapper = require('./elementWrapper/elementWrapper');
...
const waitUntil = async (condition, retryInterval, retryTimeout) => {
  ...
  while (true) {
    ...
    console.log(`currentTime : ${currentTime}\t start : ${start}\t diff : ${currentTime - start}`);
    if (currentTime - start > retryTimeout) {
      if (!actualError) {
