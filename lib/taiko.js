...
const createTarget = async () => {
  try {
    const browserTargets = await cri.List({ host: currentHost, port: currentPort }).catch(e => {
      console.trace(e);
    });
 ...
 * @param {boolean} [options.exactMatch=true] - Look for exact match, eg. { exactMatch: true | false }
 * @param {...relativeSelector} args - Proximity selectors
 * @returns {ElementWrapperList}
 */
...
function text(text, options = {}, ...args) {
  ...
  const { selector } = prepareParameters(text, options, ...args);
  const get = async () =>
