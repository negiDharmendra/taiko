...
} = require('./elementWrapper/elementWrapperUtils');
const {
  setConfig,
  defaultConfig,
...
const initCRI = async (target, n) => {
  try {
    ...
    return new Promise(r => setTimeout(r, 1000)).then(
      async () => await initCRI(target, n - 1),
    );
  }
};
 ...
 * @example
 * # switch using URL
 * await switchTo('https://taiko.gauge.org/')
 * # switch using Title
...
module.exports.switchTo = async targetUrl => {
  ...
  if (
    ...
    throw new TypeError(
      'The "targetUrl" argument must be of type string or regex. Received type ' +
        typeof targetUrl,
    );
  }
...
module.exports.dragAndDrop = async (source, destination) => {
  ...
  await doActionAwaitingNavigation(options, async () => {
    if (defaultConfig.headful) {
      await highlightElement(sourceElem.get());
      if (isElement(dest)) {
        await highlightElement(dest.get());
      }
    }
...
const calculateDestPosition = async (sourceElemNodeId, dest) => {
  ...
  if (defaultConfig.headful) {
    await overlayHandler.highlight([
      newBoundary.right,
      newBoundary.top,
      newBoundary.right,
      newBoundary.bottom,
...
const _write = async (text, options) => {
  ...
  if (defaultConfig.headful) {
    await highlightElement(activeElement.nodeId);
  }
...
module.exports.scrollTo = async (selector, options = {}) => {
  ...
  if (defaultConfig.headful) {
    await highlightElement((await findFirstElement(selector)).get());
  }
  descEvent.emit(
    'success',
